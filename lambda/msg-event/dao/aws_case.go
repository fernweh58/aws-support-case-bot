...
func CreateCaseAndChannel(c *Case) (*Case, error) {
	...
	// Prepend Lark user name to title for AWS case
	userName := GetUserName(c.UserID, c.FromChannelID)
	subject := fmt.Sprintf("[%s] %s", userName, c.Title)
...
func ResolveCase(c *Case) error {
	client := GetSupportClient(c)
	input := &support.ResolveCaseInput{
		CaseId: &c.CaseID,
	}
	_, err := client.ResolveCase(context.Background(), input)
	if err != nil {
		logrus.Errorf("failed to resolve case %s: %v", c.CaseID, err)
		return err
	}
	logrus.Infof("case %s resolved", c.DisplayCaseID)
	return nil
}
