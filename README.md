...
Lambda: msg-event (Go, ARM64)
  ...
  └── EventBridge
      ...
      ├── 转发规则 — 本地 default bus → 自定义 bus（CDK 自动创建）
      ├── 跨 Region 转发规则 — us-east-1 default bus → 部署 Region 自定义 bus（CDK 自动创建）
...
| forwardSupportEvents | EventBridge Rule | 本地 default bus → 自定义 bus 转发 |
| CrossRegionForwardRule | EventBridge Rule (us-east-1) | 跨 Region 转发 aws.support 事件（自动创建） |
| CrossRegionForwardRole | IAM Role | 跨 Region EventBridge 转发用（自动创建） |
| refreshCaseRule | EventBridge Rule | 定时轮询（默认启用） |
| EventBridgeForwardRole | IAM Role | 本地 EventBridge 转发用（自动创建） |
...
- ✅ 跨 Region 转发（aws.support 事件只在 us-east-1 产生，CDK 自动在 us-east-1 创建转发规则到部署 Region）
...
12. **跨 Region EventBridge 转发** — aws.support 事件只在 us-east-1 产生，CDK 通过 AwsCustomResource 自动在 us-east-1 创建跨 Region 转发规则
13. **扩展服务列表** — 从 10 个增加到 30 个常用 AWS 服务
14. **命令大小写不敏感** — `subject`/`Subject`/`SUBJECT` 等英文命令均可使用
15. **群聊消息附加发送者用户名** — @机器人 发的消息同步到 Case 时自动附加 `-- 用户名 via Lark`
16. **修复 billing/account 服务 category** — billing 使用 `other-billing-questions`，customer-account 使用 `other-account-issues`
17. **修复轮询空更新导致漏消息** — 只在有新 comments 时才更新 `last_comment_time`，避免空轮询跳过后续回复
18. **过滤 Bot 自身消息** — 轮询推送时过滤掉通过 Bot 发送的 comment，只推送 AWS Support 工程师的回复
19. **乐观锁防重复推送** — 使用 DynamoDB 条件更新（ConditionalUpsertCase），多个 Lambda 实例并发时只有一个能成功发送
20. **实时推送延迟等待** — EventBridge 触发后 sleep 15 秒，等待 AWS Support API 最终一致性
